...
class CasesStatisticsComponet extends Component {
  constructor() {
    super();
    this.state = {
      // name: '',
  ...
  render() {
    ...
    const tipo = [{ name: `Consulta ${body.types ? body.types.query : 0}`, value: body.types ? body.types.query : 100, fill: '#4ce1b6' },
      { name: `Solicitud ${body.types ? body.types.request : 0}`, value: body.types ? body.types.request : 100, fill: '#70bbfd' },
      { name: `Reclamo ${body.types ? body.types.claim : 0}`, value: body.types ? body.types.claim : 100, fill: '#ff4861' }];
    ...
    const plazo = [{ name: `Fuera plazo ${body.deadlines ? body.deadlines.OutTerm : 0}`, value: body.deadlines ? body.deadlines.OutTerm : 100, fill: '#4ce1b6' },
      { name: `Dentro plazo ${body.deadlines ? body.deadlines.withinTerm : 0}`, value: body.deadlines ? body.deadlines.withinTerm : 100, fill: '#ff4861' }];
      ...
      <div className="row">
        ...
        >
          <div className="dashboard__visitors-chart">
            <h1 className="dashboard__visitors-chart-title">{ body.types ? (body.types.request + body.types.claim + body.types.query) : 0 }</h1>
        ...
        >
          <div className="dashboard__visitors-chart">
            <h1 className="dashboard__visitors-chart-title">{ body.deadlines ? (body.deadlines.withinTerm + body.deadlines.OutTerm) : 0 }</h1>
            {/* <p className="dashboard__visitors-chart-number">12,384</p> */}
            <ResponsiveContainer className="dashboard__chart-pie" width="100%" height={220}>
              <PieChart className="dashboard__chart-pie-container">
                <Tooltip />
                <Pie data={plazo} dataKey="value" cy={110} innerRadius={70} outerRadius={100} />
                <Legend layout="vertical" verticalAlign="bottom" wrapperStyle={style} content={renderLegend} />
              </PieChart>
            </ResponsiveContainer>
          </div>
...
export default CasesStatisticsComponet;
